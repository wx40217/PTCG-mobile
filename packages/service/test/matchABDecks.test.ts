...
describe('梦幻ex（csve1-056）', () => {
  ...
  it('基因侵入：未接入招式列出但不可选；全部未接入时使用前整体拒绝', () => {
    ...
    // 只有未接入招式的对手：无可用模式，合并后的引擎在【混乱】硬币与事件
    // 写入前整体拒绝，不改变状态。
    ...
    const versionBefore = engine2.version;
    expectEngineError(
      () => turnCommand(engine2, 0, { type: 'attack', attackIndex: 0, target: { slot: 'active' } }),
      'unsupported-card',
    );
    const after = view(engine2, 0);
    expect(engine2.version).toBe(versionBefore);
    expect(after.pendingChoice).toBeNull();
    expect(after.activeSeat).toBe(0);
    expect(after.events.filter((event) => event.type === 'attack-used')).toHaveLength(0);
  });
});
