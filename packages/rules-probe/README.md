...
| 测试 | `npm test`（先构建，再 `node --test`，31 项） |
...
- **命令**：`{ commandId, expectedVersion, type, ... }`。`commandId` 的作用域是**认证座位**：同一座位内重复 ID 返回首次结果（`duplicate: true`），不会二次执行；同一座位内换载荷返回 `COMMAND_ID_REUSED`。另一座位即使用相同 ID，也会作为独立命令重新校验，永远不会拿到原提交者的缓存结果或私人视图。`expectedVersion` 与当前版本不一致返回 `STALE_VERSION` 且不产生状态变化。
...
| 卡（冻结效果身份） | 已实现 | 冻结数据依据 |
| --- | --- | --- |
| 古剑豹ex | 特性「战栗冷气」（战斗场，每回合 1 次，检索最多 2 张基本水能量）；招式「冰雹利刃」（水水，弃任意数量水能量，每张 60 伤害）；使用招式后回合结束（Ver 3.1.0 A-01），昏厥时先由昏厥方放置新的战斗宝可梦再开始其回合；ex 昏厥拿 2 奖赏 | `csv3c-043` 全文与招式/特性 |
| 珠贝 | 支援者，每回合 1 张；检索水宝可梦 + 物品各**最多** 1 张（两类均可放弃，类别不重复） | `csve1-138`；「物品」**不包含宝可梦道具**（2025-01-17 规则，`rule_interactions`）；找 0 张按 Ver 3.1.0 H |
| 高级球 | 物品；弃恰好 2 张手牌作为使用代价，然后检索最多 1 张宝可梦（可找 0 张） | `cbb1c-1703`；找 0 张按 Ver 3.1.0 H |
| 精灵球 | 物品；服务器硬币，正面才检索最多 1 张宝可梦（可找 0 张） | `cbb1c-1701`；找 0 张按 Ver 3.1.0 H |
...
- 没有合法候选时（例如古剑豹ex 特性发动时牌库已无基本水能量），特性直接拒绝该命令；而高级球/精灵球/珠贝 按 Ver 3.1.0 H 打开可放弃的检索选择，空候选时以 0 张结算并照常重洗牌库。
