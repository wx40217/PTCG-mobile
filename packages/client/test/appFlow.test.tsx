  ...
  it('冷启动自动重连：重入稳定房间实例，并按原 commandId 重放未确认命令', async () => {
    ...
    fake!.emit({
      ...
      view: matchView({
        sessionId: 'session-9',
        phase: 'turn-order',
        pendingChoice: { choiceId: 'choice-1', seat: 0, kind: 'turn-order', min: 1, max: 1, benchMin: 0, benchMax: 0, candidates: [], step: 1, stepCount: 1, source: 'none', descriptionZh: '测试待决选择', cardCandidates: [], modes: [] },
      }),
    });
  ...
  it('对局中断线保留页面并自动重连，重连后回到同一座位与待决选择', async () => {
    ...
    second.emit({
      ...
      view: matchView({
        ...
        version: 3,
        phase: 'turn-order',
        pendingChoice: { choiceId: 'choice-2', seat: 0, kind: 'turn-order', min: 1, max: 1, benchMin: 0, benchMax: 0, candidates: [], step: 1, stepCount: 1, source: 'none', descriptionZh: '测试待决选择', cardCandidates: [], modes: [] },
      }),
    });
    expect(await screen.findByTestId('match-turn-order-prompt')).toBeInTheDocument();
