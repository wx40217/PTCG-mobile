...
import type { ConnectResult, ConnectionFailure, DeviceIdentity, ServiceAddressPolicy } from '@ptcg/protocol';
...
describe('有效恢复身份与兼容版本', () => {
  it('连接成功后进入已连接首页并显示昵称', async () => {
    ...
    await fillProfile(user, '小智', 'http://192.168.1.8:8787');
    await user.click(screen.getByRole('button', { name: '保存并连接' }));
