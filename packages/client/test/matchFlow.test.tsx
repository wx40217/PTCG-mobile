...
import { matchCard, matchEvent, matchSide, matchView } from './matchHelpers.ts';
...
const HAND = [
  matchCard({ cardId: 'csve1-035', nameZh: '荧光鱼' }),
  matchCard({ cardId: 'csve1-057', nameZh: '月石' }),
  matchCard({ cardId: 'cbb1c-1803', nameZh: '基本水能量', kind: 'energy', classLabelZh: '能量', isBasicPokemon: false, printDisplayNumber: 'CBB1C 1803/06', hp: null, type: null }),
];
  ...
  it('公开翻面后展示双方战斗/备战与首回合归属；对方手牌身份从不渲染', () => {
    const revealed = matchView({
      ...
      you: matchSide(0, {
        ...
        active: { card: matchCard({ nameZh: '荧光鱼' }) },
        bench: [{ card: matchCard({ cardId: 'csve1-057', nameZh: '月石' }) }],
        prizeCount: 6,
      }),
      opponent: matchSide(1, {
        // 载荷中对手只有张数与公开翻面后的场上身份。
        handCount: 7,
        active: { card: matchCard({ cardId: 'csv3c-043', nameZh: '古剑豹ex' }) },
        bench: [],
        prizeCount: 6,
        revealed: true,
      }),
    ...
    expect(screen.getByTestId('match-self-bench').textContent).toContain('月石');
    ...
    expect(screen.getByTestId('match-scope-note').textContent).toContain('后续版本');
  });
