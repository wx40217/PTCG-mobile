...
未触碰其他实例。源码提交 `78c2308`（`source tree dirty lines = 0`），APK
SHA-256 `F3F4198E6C5970F14D72DDC11B9AAC158B6BC53820E48EC803DA8B7915CDE209`
（9,994,961 字节）与设备安装包逐字节一致；夹具目录版本 `13fc97aba8fc…`
（同效果身份的 V/VMAX/道具/物品别名 + 真实基本超能量）。`acceptance.log` 全部
PASS（0 FAIL）：

- **动作时机与禁用解释**：设备第 2 回合（后攻玩家的最初回合）选中 VMAX 后，
  战斗宝可梦的进化目标禁用并显示“双方玩家在自己的最初回合不能进行进化”；
  服务端同样拒绝（`action-not-allowed`）。
- **真实组合**：附勇气护符 → 附基本超能量 → 「梦中赠礼」检索物品并结束
  回合（候选 7 张只发给设备，对手原始载荷无隐藏候选；重抽公开展示的身份
  已按公开信息排除）→ 进化 V→VMAX（公开记录、对手视图一致）→ 极巨和弦
  70 点击倒对手战斗宝可梦 → 取奖赏 → `对局结束：你获胜（对手没有能放于战斗场的宝可梦）`。
- **继承与持续效果**：进化后勇气护符仍附着、最大 HP 310、3 个基本超能量
  保留；战斗记录逐条公开。
- **隐私**：按 app PID + 新鲜时间戳过滤的 logcat 无身份私钥与原生桥插件载荷；
  设备阶段结束后复验 8804/19334 映射已清理、服务已停止、设备互斥锁已释放。

证据保存在本机忽略目录 `.toolchain/issue-12-run/device/`（`acceptance.log`、
`results.json`、`01`–`06` 阶段截图、夹具目录、服务日志、拉取的 APK 等），
不随仓库提交；驱动为 `device-evolution-acceptance.mjs`（所有 CDP 调用有超时、
整体 280 秒硬上界、`finally` 有界清理并增量写证据），由
`tools/device-validation/invoke-with-device-mutex.ps1` 持有设备互斥锁执行；
APK 由 `build-apk.ps1` 在共享构建互斥锁 `Global\PTCGMobileAndroidBuild` 下构建。
本轮未通过设备验收覆盖的负路径（第一回合/名字链/刚出场、旧 choiceId、
非法候选、重进场记账等）由服务与客户端自动化测试覆盖。

**T11 仍未完成**：真机 Android 验收仍是父规格要求，本轮结论全部来自模拟器；
冻结 A/B 剩余的宝可梦与训练家效果（拖拖蚓、月石、梦幻ex、荧光鱼、珠贝、
藤树、莉佳的邀请等）属于 #13/#14，未接入卡继续拒绝；VSTAR 的「宝可梦V」
定义与特殊能量效果仍待冻结来源明确后再扩展。
