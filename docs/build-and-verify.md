...
## C/D 卡组效果、通用选择与复制招式（T13 / #14）

C/D 两套预设的全部卡牌效果在 `packages/service/src/pokemonEffects.ts`、
`trainerEffects.ts` 按冻结卡面文字逐张接入，发行目录中 C/D 的每张卡（含基本
能量）都已标记为效果已支持。本票同时把 #12 已有的
宝可梦效果、训练家效果、待决选择、视图与公开事件扩展为通用机制，不另建
同义引擎：
...
- **新机制**：营火专家（弃 1 张[火]能量代价 + 牌库顶 7 选 2）、莉佳的邀请
  （看对手手牌→放于对手备战区→互换）、捩木（弃牌区基础宝可梦与场上基础
  宝可梦互换并继承附着卡/伤害/状态/效果）、熔岩瀑布之渊（弃牌区[火]能量
  附着于备战[火]宝可梦并放 2 个伤害指示物）、古玉鱼ex（对手牌库上方弃置、
  备战多目标各附着 1 张牌库基本火能量）、古简蜗ex（按对手已获得奖赏卡张数
  的备战狙击与固定 220）、雷吉奇卡斯（五只雷吉同场的弃牌区能量附着、对
  VMAX 追加伤害）、梦幻ex（再起动抽到 3 张、基因侵入复制对手招式）、拖拖蚓
  （持续特性[钢]能量≥ 3 时最大 HP +100、刺穿 100 + 备战 30）。
- **共享接口约定**：搜索/弃牌/模式/互换延续 #11 的候选与后续动作数据；新的
  选择与目标都走同一 `PendingChoice` 投影、`choiceId + expectedVersion` 保护、
  按座位去重与重连恢复，没有旁路命令或第二套结算。

### 权威规则证据

- **古代睿智需五只同时在场**：官方简中商品文章
  [product/15767](https://www.pokemon.cn/tcg/product/15767.html)（2024-07-09）
  写明「需要在场上同时存在雷吉洛克、雷吉艾斯、雷吉斯奇鲁、雷吉艾勒奇以及
  雷吉铎拉戈的情况下方能使用」；冻结卡面文本同样列出五只。实现以
  `every` 检查五只同名在场，缺一即以可读原因拒绝，测试用夹具凑齐五只。
- **莉佳的邀请强制选择**：官方同卡 FAQ（pokemon-card.com，标准赛制）明确
  「对手手牌里有基础宝可梦时，1 张都不选→不可以」、「对手备战区没有空位时
  不能使用」、「对手手牌 0 张时不能使用」。实现：`canPlay` 在备战区满/手牌 0
  时拒绝；选择在存在可选基础宝可梦时 `min=1`，只有没有目标时才允许 0 张。
- **复制招式（基因侵入）**：冻结进阶指南 C-18「作为这个招式使用」规定执行
  被选招式的全部伤害与效果内容、无需满足被选招式费用；正文（1429-1433）同样
  规定使用的仍是原招式，因此公开的 `attack-used` 事件与延迟选择上下文始终
  保留原招式名（复制链上一直是「基因侵入」），伤害/效果内容及结算取自被复制
  招式，不把被复制招式的名字当作实际使用招式。官方同机制 Q&A
  （スミヤド/トレース→ゆびをふる，pokemon-card.com「ダブル」关键词）表明复制
  到的招式本身是复制招式时仍可继续使用；因此只要对手战斗宝可梦还有非复制
  招式，引擎就继续呈现复制选择、不设任意层数上限。当对手战斗宝可梦的唯一
  可选招式就是「基因侵入」自身时，复制只会再次要求同一选择，形成无状态变化
  且没有出口的强制循环：#13 的有界来源检索仍未找到直接官方裁定（冻结 C-18
  只规定执行与保留原招式名、没有终止规则；2023 Tournament Handbook 5.8.3.2
  只涉及比赛时间结束后的额外回合与裁判处理），因此引擎只对这一真正封闭的
  循环按“无效果收招”处理并保留原招式名公开记录。这是合并后的**工程决定
  （实现行为）**，不是官方裁定；已验证范围限于无状态变化、无永久待决选择，
  存在出口时不设任意层数上限，不得声称为官方规则或完成父规格整体验收；
  全部未接入的复制目标在消耗【混乱】硬币前整体拒绝，不改变状态。
- **原子性**：复制资格（对手战斗宝可梦存在至少 1 个已接入招式）在
  `planAttack` 登记阶段校验，先于【混乱】硬币、公开事件与 `deferredAttack`
  写入；不支持的选择返回 `unsupported-card` 且不改变版本/事件/随机。
...
- `packages/service/test/matchDecksABFlow.integration.test.ts` 含**跨线 A vs C**
  ...
  邀请的强制选择也在真实服务上验证（0 张提交返回 `illegal-choice` 且待决
  选择保留）。
- 客户端 `matchController`/`MatchScreen` 新增三类选择命令与界面（卡片候选、
  目标列表、复制招式列表与禁用原因），目录支持数量改为按支持清单动态断言。

### 基本能量与按卡组就绪（T13 / #14 与 T12 / #13 合并后）

`packages/service/src/energyEffects.ts` 声明四种基本能量的效果身份已由引擎的
通用附着/支付/弃置路径支持，并写入发行支持清单，因此四套预设的每张卡（含
基本能量）在目录中都标记为 `effectSupported`。`validateDeck` 的按卡组就绪不再
依赖整份目录的 `supportPolicy.playable`：只要卡组规则合法、每张卡都已支持、
且 `engineIntegration === 'integrated'`，卡组即 `ready=true`；#13 的 A/B 效果与
#14 的 C/D 效果合并后，A/B/C/D 四套预设均为 `ready=true`，而整份目录仍因
47 张中仍有效果未接入（如特殊能量）保持 `playable=false`。回归：协议
`deck.test.ts`（四套预设均就绪、未支持卡拒绝、引擎未接入拒绝）、服务
`deckValidation.integration.test.ts`（A/C 文本往返后 `ready=true`）、
`trainerSupport.test.ts` 与工具链 `build-catalog.test.mjs` 的注册表/目录双向核对。

### 与 #13 的合并（本次集成）

本分支与 `main`（#13，`999f69a`）合并后，A/B 与 C/D 的效果注册表、基本能量、
通用待决选择、复制链与卡牌就绪检查共用同一引擎与发行支持清单：低层取 #13 的
`prepareAttackEffect`/`applyPreparedAttack` 风格暂存登记与 `choose-mode` 复制
流程，同时保留 #14 的 `mill` 暂存操作、备战狙击/牌库能量附着/弃牌区选卡等
C/D 回调、`use-stadium` 与基本能量客户端面板；未接入招式在【混乱】硬币前整体
拒绝（原子性），全部 A/B/C/D 预设均为 `ready=true`。回归即上节测试矩阵与 4 项
跨线预设对局、1 项跨线真实服务开局。
...
服务端校验 `legal:true, ready:true, problems:[]`）后完成整局：
...
“描述 + 候选卡 + 控件”键控、最多 3 次且应答后先等待视图变化（避免陈旧 DOM
重复提交）；以服务端 `version`/`choiceId` 的 45 秒无进展判定权威失败并导出
DOM 诊断；`attach-target` 分支遵守失败黑名单；新增道具面板步骤；对手端在
...
原招式名公开记录，保留一切有非复制出口时的继续选择与不设层数上限。这是
已验证的实现行为（无永久待决选择、无状态变化），不是官方规则证明；两局
设备验收也不构成该规则的官方证明。文档与报告不得把该行为称为官方裁定或
完成父规格整体验收。

共享前置：#13（`368f527`/`1a60489`）的 `packages/service/src/energyEffects.ts`
与基本能量支持已在本票提交 `30eb9ff` 中按只读比较结果独立集成（未合并、未
cherry-pick）；本票不再以夹具标记绕过就绪门槛。

**本票交付边界**：C/D 卡牌效果、通用选择机制与完整对局已由自动化测试、
真实 WebSocket 边界与上述两局 MuMu Android 模拟器整局验收覆盖（物理 Android
真机验收仍待父规格）；复制闭环为合并后的工程决定（非官方裁定，见上），2D
牌桌的其余交互限制仍由父规格发布门槛（含 #17）决定。
...
`data/effects/zh-cn-standard-2025-06-05-supported-effects.json` 现列 28 个效果
身份（#11/#12/#13 的 A/B 卡、#14 的 C/D 卡与四种基本能量合并后），目录
`playable=false` 仍表示全 47 条未全部接入（如特殊能量），未接入卡继续以
...
- **A/B 剩余训练家效果**：藤树（最多 3 张「连击」基础宝可梦直入备战区，同时受
  ...
  宝可梦放于对手备战区并与战斗宝可梦互换；对手没有手牌或备战区已满时整体拒绝
  且不消耗支援者/不展示手牌——官方同卡 FAQ 明确备战区已满时不能使用）；珠贝
  （依次检索[水]宝可梦与真正的物品各 1 张，两步都允许 0 张，且只在整段效果结束
  后重洗 1 次；宝可梦道具不属于「物品」）。
...
- **生命周期与原子性**：复制招式的延迟上下文只在本次招式结算期间存在；立即结算
  的复制招式在验证成功后先释放上下文再结算，下一回合的选择（如莎莉娜弃抽、特性）
  不会误收旧招式或提前换手。`resolveModeChoice` 在消费待决选择前完成全部校验
  （复制计划、目标、可弃手牌），登记失败保持版本/待决选择/视图/随机完全不变，
  已有公开回归（含真实 WebSocket）。
...
- `packages/service/test/matchABDecks.test.ts`（22 项）逐卡覆盖正常/失败/可选
  分支：特性次数与条件、复制招式的固定伤害与多步选择、复制上下文生命周期、
  复制计划失败的原子性、强制镜像收招、弃牌代价与抽牌、被动 HP 修正、备战目标与
  弱点/抗性、任意次附着、标签检索、对手手牌隐私与互换、备战区已满拒绝、分步
  检索单次重洗、四种基本能量附着。需要本冻结环境没有的「随心游动」目标或
  [钢]能量时使用带明确夹具身份的测试卡，不改发行目录。
- `packages/service/test/matchDecksABFlow.integration.test.ts`（7 项）让新增
  待决流程逐一通过真实服务 + 两个真实 WebSocket 客户端，并断言服务端分发不落
  入 `room-error`：莉佳的邀请（对手手牌候选只发给选择者）、基因侵入
  （`choose-mode`/立即复制后下一回合莎莉娜弃抽/镜像无终止路径收招）、刺穿
  （`switch-opponent` 后续伤害）、循环抽取（攻击发起的 `discard-hand`）、
  珠贝（两步检索 + 单次重洗）、A/B 预设可准备开局。
- `scripts/e2e-decks-ab.mjs`（24 项）用发行目录预设 A/B 与一副合法混搭改组跑完
  ...
  5/5 完成且每场均含真实招式（6–15 次），胜负理由包括 prizes / no-pokemon /
deck-out。

### Android 客户端完整对局验收（2026-09-21，#13）

在 MuMu Player 12（Android 12 / SDK 32）实例 0（`127.0.0.1:16384`）上，用 ADB +
WebView CDP（回环端口 19335）与构建后的真实服务（回环端口 8805）完成真实 APK +
主机进程第二客户端的整局流程；所有设备操作在
`Global\PTCGMobileDeviceValidation` 互斥锁内执行，APK 在
`Global\PTCGMobileAndroidBuild` 下从 `0bbb850` 工作树构建。设备端复制发行预设 A
为合法草稿（60 张，不改数量/身份/支持标记），主机第二客户端使用一副合法改组
（全部来自冻结 A/B 已支持卡池，排除梦幻ex 以避开只有「基因侵入」的合法镜像
循环）；设备端经真实服务端校验为“可以正式对战”。

- **APK 一致性**：源码提交 `0bbb850aafd3975f4de63632c80fd5164891d7cf`
  （`source tree dirty lines = 0`）；本地与设备包 SHA-256 均为
  `DF7237D4EEAF69F66210FAFCA4A080320256DCBA6AA5DB1B747C12F8E49F0CA1`
  （9,994,969 字节），安装后逐字节核对。
- **完整对局**：设备（座位 0，预设 A）对主机（座位 1，合法改组）跑完整局，
  终局为 `winner=1 / no-pokemon`；公开事件含 `attack-used=2`（「刺穿」）、
  `pokemon-knocked-out=2`、`prizes-taken=2`、`replacement-placed=1`；设备端
  11 步真实 UI 操作（盖放/特性/检索/附能/取奖赏等），其中 8 次是受控 input 在
  ADB 触控未触发 React change 时的 DOM `click()` 兜底（已在 `match-summary.json`
  显式计数）。
- **隐私与清理**：按 app PID + 新鲜时间戳过滤的 logcat 无身份私钥、无
...
- **目录版本边界**：设备运行时服务加载的目录版本为
  `d6da2b4c6e9cc84b80e4593bbf59a5461e9daff43e938a4087e78ae4e6c12afc`；
  此后仅效果支持清单中「基因侵入」的说明文字更新（无卡牌/支持标记/行为变化），
  最终提交目录为 `7cb49311b591…`。

**T12 交付边界（父规格仍未整体验收）**：C/D 卡牌与特殊能量仍未接入，目录整体
仍未 `playable`；Android 结论只来自 MuMu 模拟器（未在物理真机验证），设备端
卡组是发行预设 A、主机端是合法改组而非双方预设对局，且牌桌交互仍是当前列表/
表单界面（2D 互动牌桌由 #17 实施）；「海之伴奏」正常路径与「营养铁质」的[钢]能量
条件在本冻结卡池内没有可用的生产目标/能量，已由显式夹具测试覆盖，生产可达分支
是公开拒绝/不变；真机验收仍是父规格要求。
