...
- **通用待决选择**：`MatchPendingChoiceView` 统一暴露所有者座位、步骤 `step/stepCount`、
  ...
  其中不满足卡面文字的卡以 `selectable=false` 展示但不可选。
...
- **客户端**：`matchController` 增加出牌、竞技场与四类选择命令；`MatchScreen`
  ...
  提交、候选卡放大（完整简中卡面文字；卡图可用时按需加载，可再放大）、被查看但
  不可选的候选禁用并标注、模式选择与互换表单、竞技场持续状态展示。

```bash
npm test                     # 协议 133 / 服务 187 / 客户端 201，共 521 项
npm run test:e2e:trainers    # 真实服务 + 两客户端：5 场聚焦对局、43 项检查
```
...
- **超级球**：查看牌库顶 7 张（全部 7 张作为私人候选、只有宝可梦可选）→ 提交 0 张
  结束检索、不公开结果但重洗牌库 → 继续对局；隐藏别名候选在对手原始载荷中出现 0 次。
...
共 43 项通过；发行目录仍只标记 7 张训练家卡、`playable=false`。
