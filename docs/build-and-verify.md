...
- **高级球**：弃 2 张手牌代价 → 牌库检索 → 公开展示所选的 1 张 → 重洗 → 继续对局；
  ...
  展示前不出现在第二客户端原始载荷；同一身份若已在更早的公开区域/公开事件
  （如开局重抽展示手牌）出现，隐私断言按接收方真实视图排除该公开身份后再判定；
  检索答案精确重传不重复洗牌/抽卡。
- **超级球**：查看牌库顶 7 张（全部 7 张作为私人候选、只有宝可梦可选）→ 提交 0 张
  结束检索、不公开结果但重洗牌库 → 继续对局；被查看的隐藏别名身份同样只在
  从未公开过时才必须在对手原始载荷中缺席。
...
共 43+ 项通过（数量随重抽与硬币次数变化）；连续复跑可覆盖“候选已在更早公开
载荷中出现”的公开身份排除分支；发行目录仍只标记 7 张训练家卡、`playable=false`。
