...
npm test                    # 全部单元与集成测试（协议 133 项 / 服务 179 项 / 客户端 200 项，共 512 项）
...
- **通用待决选择**：`MatchPendingChoiceView` 统一暴露所有者座位、步骤 `step/stepCount`、
  ...
  卡牌投影）只发给选择者，不进入对手载荷、公开记录或错误；没有通用取消入口，
  可选零张、检索失败与无目标都按具体卡面处理。
- **逐卡行为**：高级球先支付 2 张其他手牌代价再检索（代价不因检索失败退还）；
  精灵球硬币正面才检索、反面不检索也不洗牌；超级球只看牌库顶 7 张；等级球 HP≤90；
  鼓励信需要上一个对手回合己方宝可梦昏厥（回合开始时结转、下次自己的回合可用）；
  莎莉娜二选一（弃 1..3 张后抽到手牌 5 张 / 互换对手备战区「宝可梦V」与战斗宝可梦）；
  深钵镇双方各自每回合 1 次检索基础非规则宝可梦直接放于备战区。
...
- **客户端**：`matchController` 增加出牌、竞技场与四类选择命令；`MatchScreen`
  ...
  提交、候选卡放大（完整简中卡面文字；卡图可用时按需加载，可再放大）、模式选择
  与互换表单、竞技场持续状态展示。

```bash
npm test                     # 协议 133 / 服务 179 / 客户端 200，共 512 项
npm run test:e2e:trainers    # 真实服务 + 两客户端：4 场聚焦对局
```

### 训练家端到端（2026-09-20，T10）

`npm run test:e2e:trainers` 在真实服务进程 + 两个真实 WebSocket 客户端上跑 4 场
对局；夹具目录用“同效果身份的多个别名印刷版本”保证目标训练家卡出现在手牌，
服务端执行的是生产注册表中的真实效果身份：

- **高级球**：弃 2 张手牌代价 → 牌库检索 → 公开展示所选的 1 张 → 重洗 → 继续对局；
  隐藏候选在展示前不出现在第二客户端原始载荷；检索答案精确重传不重复洗牌/抽卡。
...
共 36 项通过；发行目录仍只标记 7 张训练家卡、`playable=false`。

### 训练家设备验收（待续，2026-09-20，T10）

按本票要求准备了 MuMu Player 12（Android 12 / SDK 32，127.0.0.1:16384）的真实
APK 驱动 .toolchain/issue-11-run/device/device-trainer-acceptance.mjs：真实 ADB
触控 + WebView CDP 回环 19332、服务回环 8802，设备阶段在
Global\PTCGMobileDeviceValidation 互斥锁下运行，流程为设置连接 → 复制夹具预设
（36 个同效果身份的别名印刷版本）→ 建房 → 主机第二客户端加入/选卡组/准备 →
设备选卡组/准备 → 自适应开局 → 设备真实点击使用高级球 → 弃 2 张手牌代价 →
牌库检索 → 放大候选卡 → 明确提交 → 公开展示/重洗 → 继续回合。

本轮多次尝试取锁均返回 DEVICE_MUTEX_BUSY（并行的 T14 恢复验收仍持有设备锁，
其设备日志停在等待重连横幅且进程未退出），按约定没有轮询、没有杀死持有者、没有
执行任何设备动作；因此本票尚缺真实 Android 证据，设备阶段待管理器协调锁空闲后
恢复执行。已构建的本地 debug APK 为 9,987,089 字节，SHA-256
F6CFD80ED72161D920960733023138875E78EE006EDA1A5B311CA205A7154609（源码提交
`f669cce`），设备驱动会先安装并逐字节核对哈希，再进入上述流程。
