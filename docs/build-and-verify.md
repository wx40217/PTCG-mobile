...
- **复制招式（基因侵入）**：冻结进阶指南 C-18「作为这个招式使用」规定执行
  ...
  招式，引擎就继续呈现复制选择、不设任意层数上限。只有当前全部已接入招式
  都是复制类效果（真正闭合、无任何非复制出口）时，才以**暂定引擎边界**
  以原招式名公开记录后收招。该闭环处理没有直接官方裁定：冻结 C-18 只规定执行
  与保留原招式名、没有终止规则；2023 Tournament Handbook 5.8.3.2 只涉及
  比赛时间结束后的额外回合与裁判处理，不能作为自动平局/终止依据。因此闭环
  行为明确标为“未验证的暂定限制、保留所有出口路径”，不能声称已满足完整
  规格（见下节与最终报告）。
...
通用附着/支付/弃置路径支持，并写入发行支持清单，因此 C/D 预设的每张卡（含
基本能量）在目录中都标记为 `effectSupported`。`validateDeck` 的按卡组就绪不再
依赖整份目录的 `supportPolicy.playable`：只要卡组规则合法、每张卡都已支持、
且 `engineIntegration === 'integrated'`，卡组即 `ready=true`；整份目录此时仍因
A/B 专属效果未接入而 `playable=false`。回归：协议 `deck.test.ts`（C/D 就绪、
A/B 不就绪、未支持卡拒绝、引擎未接入拒绝）、服务 `deckValidation.integration.test.ts`
（D 文本往返后 `ready=true`）、`trainerSupport.test.ts` 与工具链
`build-catalog.test.mjs` 的注册表/目录双向核对。
...
**复制类招式闭环终止：未验证，不是本票结论。** 官方并未检索到“基因组黑客”
把仅有的可用招式复制成闭环时的终止裁定；实现仅保留一个显式标记为临时的
闭环保护（所有可用招式都是复制招式、没有非复制出路时以原招式名结算、不
追加效果），并保留一切存在非复制出路的选择继续呈现。两局设备验收不构成
该规则的官方证明；在规则裁定补齐前，不将该行为称为已验证或完成整体验收。
