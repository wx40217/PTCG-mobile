...
- **复制招式（基因侵入）**：冻结进阶指南 C-18「作为这个招式使用」规定执行
  被选招式的全部伤害与效果内容、无需满足被选招式费用；官方 FAQ 对
  「选择的招式的处理无法执行」（如ミュウex 复制エンジェライト时已使用过）
  给出「ワザの処理はおこなわず、ワザを終わります」。镜像时选中另一个
  「基因侵入」的复制处理无法产生新的结算状态：选择本身仍然合法并公开呈现，
  引擎以原招式名公开记录 0 伤害并收招，不递归创建永久待决选择。该自我参照
  终止没有逐字的官方同卡裁定，是对上述冻结 C-18 与官方收招模板的规则解释，
  已在代码与支持清单中注明来源边界。
