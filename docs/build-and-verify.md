...
npm test                    # 全部单元与集成测试（协议 89 项 / 服务 34 项 / 客户端 96 项）
...
≥1 张基础宝可梦、进化前置在卡组内、环境标记合法，并只有所有效果已接入才给出
就绪。当前冻结目录全部为“效果未接入”，因此四套预设都只能编辑与预览，不能开局。
...
导入也接受唯一卡名或卡牌编号的简化行；未知编号、同名歧义、身份与环境不符都
整体失败，界面不会覆盖原卡组。
