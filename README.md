...
（T04）；卡组已贯穿「预设/复制 → 草稿编辑与持久化 → 版本化文本导入导出 →
客户端/服务端同源合法性校验」（T05）；朋友房间已贯穿「6 位房间码邀请 →
稳定房间实例与预期版本路由 → 两个认证座位 → 服务端校验并固定卡组 →
双方基于同一目录修订准备建立唯一对局会话」（T06），
开局后的规则结算尚未实现。可重复的构建、测试与 APK 命令，以及当前的
