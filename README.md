...
面向简体中文环境的独立 PTCG 安卓客户端：离线单人对战、朋友房间、自由组卡与自动结算。

启动直接进入单人首页：独立昵称、三套冻结预设与三位对手全部开放，无需服务地址、
联机身份或预热卡库缓存。对局沿用横屏 2D 牌桌，文字卡面随包提供；支持自动存档、
继续待决选择、确认放弃、确认认输、结算后再来一局或更换对手。台词偏好与本地胜负
独立保存。朋友联机入口保留原四套预设、自由改组、房间与网络恢复行为。
单人首版不接受自由改组；浏览器/组件验证不替代物理 Android 与用户试玩验收。
...
对局会话」（T06）；开局流程已贯穿「服务端随机决定先后攻选择权 → 获选玩家明确选择 →
真实洗牌与 7 张手牌 → 无基础宝可梦展示/重抽（共同重洗不计 5.d.）与按对手
单独重抽次数的可选补抽 → 盖放战斗/备战宝可梦 → 6 张奖赏卡 → 对战开始前
剩余基础宝可梦的最终备战 → 公开翻面 → 唯一首回合」（T07）；真实交替回合已贯穿
「回合开始抽牌 → 基础宝可梦进备战区 / 每回合 1 张附能 / 每回合 1 次撤退 →
招式按基础伤害、弱点、抵抗顺序结算并结束回合」（T08）；完整结算已贯穿
「宝可梦检查与特殊状态（中毒/灼伤/睡眠/麻痹/混乱及其共存/替换/随机）→ 昏厥时
宝可梦与附着卡进弃牌区、同时/多只昏厥、强制升前与奖赏数量 → 最后奖赏、
无后备与回合开始抽空判败、同时满足胜负条件的判定表与平局 → 确认认输的唯一终态
→ 返回原房间重新准备新局」（T09）；首批训练家卡已贯穿「物品/支援者/竞技场类别
限制与先攻首回合限制 → 高级球弃 2 张手牌代价与检索、精灵球硬币、超级球牌库顶
7 选 1、等级球 HP≤90、鼓励信条件检索、莎莉娜弃牌抽牌与对手换位、深钵镇持续
竞技场 → 按座位私密候选与依法公开结果 → 多步选择明确提交」（T10），未接入的
卡牌效果仍不能用于正式对局。
可重复的构建、测试与 APK 命令，
以及当前的
