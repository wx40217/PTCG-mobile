...
目录已贯穿「冻结资料 → 版本化目录服务 → 安卓搜索/详情/资源样本/离线缓存」
（T04）；卡图资源由独立流程准备为版本化资源包，客户端按内容哈希按需缓存，
更新失败保留旧图、清除图片缓存不影响身份（T15）；卡组已贯穿「预设/复制 →
草稿编辑与持久化 → 版本化文本导入导出 → 客户端/服务端同源合法性校验」
（T05）；朋友房间已贯穿「6 位房间码邀请 → 稳定房间实例与预期版本路由 →
两个认证座位 → 服务端校验并固定卡组 → 双方基于同一目录修订准备建立唯一
对局会话」（T06），开局后的规则结算尚未实现。可重复的构建、测试与 APK 命令，
以及当前的
